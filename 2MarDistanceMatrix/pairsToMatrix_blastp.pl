...
# This script was a little bit modified compared to the initial one.
...
my ($gb_clusternum_date) = build_gb_clusternum_desc($clusterFile); # key = gb, key2 = clusternum, val = desc
...
foreach my $m1 (@modules) {
  ...
  foreach my $m2 (@modules) {
    my $sim = $m1_m2_sim{$m1}{$m2} || 0;
    my $sim2 = $m1_m2_sim{$m2}{$m1} || 0;
...
sub build_m1_m2_sim{
  ...
  while(<IN>) {
    ...
    $m1_m2_sim{$label1}{$label2} = $sim;
  }
  return(%m1_m2_sim);
}
    

sub build_gb_clusternum_desc{
  my $file = shift;
  my %desc =  ();
  my %date =  ();
  my %domains =  ();
