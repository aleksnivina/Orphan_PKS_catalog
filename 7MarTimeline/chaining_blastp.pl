...
my $outfile1 = "$file.redundant_chained.cutoff$cutoff.txt";
my $outfile2 = "$file.nonredundant_chained.cutoff$cutoff.txt";
	...
	foreach my $b(sort keys %similarities) {
		...
		if ($similarities{$a}{$b}>=$cutoff) {
			...
			# and also add all potential records that are redundant to $a, if they have not already been saved as redundant to $b or $d
				...
				my $previous_chain=$redundant{$d};
				# redundancies that have been recorded for $a
				my $redundant_chain=$redundant{$a};
				...
				unless ("$a" eq "$d") {
					# if some records redundant to $a have already been saved as redundant to $d, then 
					my $additional_chain=additional_redundancy($previous_chain,$redundant_chain);				
					$redundant{$d}="$previous_chain\t$a\t$additional_chain";
					delete 	$redundant{$a};
				}
			}	
		}
