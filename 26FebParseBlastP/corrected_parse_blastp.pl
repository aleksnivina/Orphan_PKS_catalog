...
foreach my $file(@files_subset) { ############################## parallelize
  ...
  # get clusternum
  $file =~ /(.*)\.cluster(\d+)\.(.*)\.out/ || print "Filename for $file didn't have expected format\n"; # !!mod!!
  my $gb1 = $1;
  ...
  if ($first_two_letters eq "00") {
  	$cluster1=substr($cluster1_prelim,2,1);
  }
  elsif ($first_letter eq "0") {
  	$cluster1=substr($cluster1_prelim,1,2);
  }
  else {
  	$cluster1=substr($cluster1_prelim,0,3);
  }
  ...
  while(<IN>) {
	...
	# print "read query $query, subject $subject\n";
  	...
  	if (("$subject" ne "$prev_subj") and ("$prev_subj" ne "")) {
	    ...
	    record_score($gb1,$cluster1,$query_length,$gb2,$cluster2,\%hits);
  		%hits = ();
  	}
