...
# Plot known clusters only; 
# python plotting_dendrograms_all.py -show_all Yes -show_known Yes -only_show_known No -show_accession Yes -show_species Yes -show_date No -show_type Yes -show_domains Yes
...
if show_all=="No":
    ...
    with open("parsed_scores.all_final.redundant_chained_new.cutoff0.9.txt",'r') as redundancy_file:
    ...
    with open("parsed_scores.all_final.nonredundant_chained_new.cutoff0.9.txt",'r') as nonredundancy_file:
...
with open("./clusterTableNonRedundantNonSimilar.6dbs.prot_len.correct_domains.txt",'r') as cluster_table_file:
...
if show_known=="Yes":
    ...
    if show_all=="Yes":   
        ...
        with open("known_clusters_MIBig_and_clusterTable_matched_to_all_leaves_edited.txt",'r') as known_file:
        ...
        with open("known_clusters_MIBig_and_clusterTable_matched_to_main_edited.txt",'r') as known_file:
            known_cluster_list=known_file.readlines()
            for line in known_cluster_list:
                cluster_info=line.split("\t")
                cluster_id=cluster_info[0].strip(":")
...
with open("../5_creating_distancematrix/dataMatrix_blastp.txt",'r') as distance_file:
...
for k in range(1,len(distance_data)):
    ...
    if show_accession=="Yes":
        curr_description+=accession+" cluster "+clusternum+" "
    if show_species=="Yes":
    ...
    counter+=1
    labels.append(curr_description)
...
if only_show_known=="Yes":
    interesting_labels=["KU568466 1","AY118081 1","AB089954 1","AY509120 1","MF033535 1","KP997155 1","KP997155 1","AR897801 1","EU220288 1","AF016585 1"]
    ...
    # print "interesting labels",interesting_labels_full[0:5]
    ...
    for item in id_labels:
        if show_all=="Yes":
            if item in known_clusters_all:
                ...
                full_label=item_data[0]+" cluster "+item_data[1]
    ...
    print "known labels",known_labels_full[0:5]
...
if only_show_known=="Yes":
    ...
    fig,axes = pyplot.subplots(figsize=(10,150))
else:
    fig,axes = pyplot.subplots(figsize=(10,100))
...
if only_show_known=="Yes":
    ...
    axes.tick_params(axis='y', which='major', labelsize=1.5)
else:
    axes.tick_params(axis='y', which='major', labelsize=2)
...
if show_all=="Yes":
    ...
    axes2.plot([0.1,0.1],[0,100],color="grey",linewidth=1)
