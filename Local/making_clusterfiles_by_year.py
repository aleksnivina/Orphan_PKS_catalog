...
2018:clustertable2018}

with open("../1_extracting_prot_sequences/clusterTableNonRedundantNonSimilar.6dbs.prot_len.txt",'r') as clustertable_final:
	counter=True
	...
	while counter:
		...
		if len(line)>0:		
			...
			try:
				...
				main_years=[0 for i in range(1994,2019)]
				redundant1_years=[0 for i in range(1994,2019)]
				redundant2_years=[0 for i in range(1994,2019)]
				curr_names=[[] for i in range(1994,2019)]

				# saving the main entry name and date, if it's earlier or equal to a year
				main_entryname=data[0]
				main_date=int(data[2])
				main_clusternum=int(data[4][8:])
				for year in years:
					if main_date<=year:
