...
with open("known_clusters_MIBig_and_clusterTable_matched_to_main.txt",'w') as output_file:
	with open("./clusterTableNonRedundantNonSimilarNonSequenceSimilar.6dbs.prot_len.correct_domains.txt",'r') as final_cluster_file:
        ...
        for n in range(1,len(final_cluster_list)):
            ...
            main_cluster_n=str(int(line[4][7:]))
            ...
            for curr_cluster in cluster_ids:
                ...
                if curr_cluster in known_assembly_line_PKS_dict :
                	known_products+=known_assembly_line_PKS_dict[curr_cluster]+", "
            if len(known_products)!=0:
	            output_file.write(main_accession+" "+main_cluster_n+"\t"+known_products[:-2]+"\n")
